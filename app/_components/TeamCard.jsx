"use client";

import Link from "next/link";
import { capitalize } from "../_helper/capitalize";

import React from "react";
import {
  CardBody,
  CardContainer,
  CardItem,
} from "@/app/app/_components/3d-card";

export function TeamCard({ name, description, image }) {
  return (
    <CardContainer className="inter-var">
      <CardBody className="bg-gray-50 relative group/card  dark:hover:shadow-2xl dark:hover:shadow-emerald-500/[0.1] dark:bg-black dark:border-white/[0.2] border-black/[0.1] w-100  h-auto rounded-xl p-6 border  ">
        <CardItem
          translateZ="50"
          className="text-xl font-bold text-neutral-600 dark:text-white"
        >
          {capitalize(name)} Team
        </CardItem>
        <CardItem
          as="p"
          translateZ="60"
          className="text-neutral-500 text-sm max-w-sm mt-2 dark:text-neutral-300"
        >
          {description}
        </CardItem>
        <CardItem translateZ="100" className="w-full mt-4">
          <img
            src={image || "/events/web.png"}
            height="1000"
            width="1000"
            className="h-60 w-full object-cover rounded-xl group-hover/card:shadow-xl"
            alt={`${name} team`}
          />
        </CardItem>
        <div className="flex justify-end items-center mt-10">
          <CardItem
            translateZ={20}
            as="button"
            className="px-4 py-2 rounded-xl bg-success text-xs font-bold"
          >
            <Link href={`/teams/${name}`}>Know more </Link>
          </CardItem>
        </div>
      </CardBody>
    </CardContainer>
  );
}

export default TeamCard;
