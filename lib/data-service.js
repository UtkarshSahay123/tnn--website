...
function normalizeEvent(event) {
  ...
  const image = event.image?.trim().replace(/^"(.*)"$/, "$1");
  const hasLocalImagePath = /^[a-zA-Z]:[\\/]/.test(image ?? "");

  return {
    ...event,
    image: hasLocalImagePath ? "" : image,
  };
}
...
function normalizeMember(member) {
  ...
  const image = member.image?.trim().replace(/^"(.*)"$/, "$1");
  const hasLocalImagePath = /^[a-zA-Z]:[\\/]/.test(image ?? "");
...
export async function getEventDetails(id) {
  try {
    ...
    if (error) {
      console.error("Supabase getEventDetails error:", error);
      return null;
    }

    return normalizeEvent(data);
  } catch (err) {
    console.error("Supabase getEventDetails fetch failed:", err);
    return null;
  }
}
